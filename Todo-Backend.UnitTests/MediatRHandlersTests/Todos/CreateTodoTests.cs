...
public class CreateTodoHandlerTests
{
    ...
    {
        _handler = new CreateTodoHandler(_mapperMock.Object, _todoRepositoryMock.Object, _loggerMock.Object);

        // важно: чтобы await AddAsync не упал (по умолчанию Moq вернет null для Task)
        _todoRepositoryMock
    ...
    public async Task Handle_ShouldReturnOkResult_WhenTodoIsCreated()
    {
        ...
        Assert.Equal(todoId, result.Value.id); // свойство 'id' в record с маленькой буквы
    ...
    public async Task Handle_ShouldReturnFailResult_WhenMapperReturnsNull()
    {
        ...
        Assert.Contains(result.Errors, e => e.Message == "CannotConvertNullToNews");
